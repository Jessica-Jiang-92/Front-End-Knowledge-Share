...
本来按照最合理的逻辑来说，旧的第一个`vnode` 是应该直接完全复用 新的第三个`vnode`的，因为它们本来就应该是同一个`vnode`，自然所有的属性都是相同的。但是在进行子节点的 `diff` 过程中，会在 旧首节点和新首节点用`sameNode`对比。 这一步命中逻辑，因为现在新旧两次首部节点的 `key` 都是 `0`了，
