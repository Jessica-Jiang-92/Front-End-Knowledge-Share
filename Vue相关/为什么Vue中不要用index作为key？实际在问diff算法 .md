...
```
有一个细节需要注意，Vue 对于组件的 `diff` 是不关心子组件内部实现的，它只会看你在模板上声明的传递给子组件的一些属性是否有更新。也就是和`v-for`平级的那部分，回顾一下判断 `sameNode` 的时候，只会判断`key`、 `tag`、是否有`data`的存在（不关心内部具体的值）、是否是注释节点、是否是相同的`input type`，来判断是否可以复用这个节点。
